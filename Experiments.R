...
# Build the cache beforehand
unique_locations <- location_data %>%
  pull(location) %>%
  unique()

# Pre-cache geocoded locations to avoid repeat calls
cache <- map_dfr(unique_locations, ~ geocode_location(.x, cache))
...
expanded_location_data <- location_data %>%
  ...
  mutate(location = str_replace_all(location, "\\(([^)]*),([^)]*)\\)", "(\\1 \\2)"),
         location = str_replace_all(location, "\\((.*?)\\)", " \\1"),
         location = str_replace_all(location, ",(\\S)", ", \\1")) %>%
  separate_rows(location, sep = ", ") %>%
  
  # Use `future_map` for parallel processing
  mutate(geo_data = future_map(location, ~ geocode_location(.x, cache))) %>%
  
  mutate(geo_data = map(geo_data, ~ rename_with(.x, ~paste0("geo_", .)))) %>%
  unnest(geo_data) %>%
  arrange(row_id) %>%
  select(-row_id)
